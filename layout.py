...
# ================================
# LEITURA DOS DADOS E CÁLCULOS INICIAIS
# ================================
...
df_churn['Faixa_etaria'] = pd.cut(df_churn['Idade'], bins=bins, labels=labels, right=False)
ttidade = df_churn['Faixa_etaria'].value_counts().sort_index()

# Total de Churn por Estado
total_valores_estados = (
    df[df['Status_do_Churn'] == 1]
    .groupby('Estado')['Receita_Total']
    .sum()
    .rename('Total_Receita_Churn_Estados')
    .sort_values(ascending=False)
).reset_index()

# Agrupamento por faixa etária para clientes
...
# Distribuição preditiva por faixa etária e tempo de contrato
...
# Mapa dos estados (exemplo de dicionário de todos_estados)
...
total_valores_estados = total_valores_estados  # já reset_index feito acima
